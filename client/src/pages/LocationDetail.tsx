...
import { BarChart2, CheckSquare, ExternalLink, FileText, MapPin, Square } from "lucide-react";
    ...
    <PortalLayout>
      <div className="px-6 py-8 max-w-5xl">
        ...
        <div className="mb-8">
          ...
          >
            {loc.city}, {loc.state} · Franchise reporting territory
          </div>
        ...
        <section className="mb-10">
          ...
          >
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div>
                  <div className="text-sm font-semibold" style={{ color: "white", fontFamily: "Inter, sans-serif" }}>
                    {loc.name} — Territory Dashboard
                  </div>
                  <div className="text-xs" style={{ color: "oklch(0.75 0.06 145)", fontFamily: "Inter, sans-serif" }}>
                    Source-aware operational and digital reporting
                  </div>
                </div>
              </div>
              <Link
                href={`/dashboard/${loc.id}`}
                className="flex items-center gap-2 px-4 py-2 rounded-sm text-xs font-bold transition-opacity hover:opacity-80 flex-shrink-0"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-0" style={{ borderBottom: "1px solid oklch(0.88 0.012 80)" }}>
              <div className="p-5" style={{ borderRight: "1px solid oklch(0.88 0.012 80)" }}>
                <div className="text-xs font-semibold tracking-widest uppercase mb-3" style={{ color: "oklch(0.52 0.016 80)", fontFamily: "Inter, sans-serif" }}>Reporting sources</div>
                <div className="space-y-2">
                  {[
                    { icon: <FileText size={13} />, label: "Google Drive workbook — active aggregate status and coverage shown in context" },
                    { icon: <BarChart2 size={13} />, label: "Google Analytics 4 — direct and completed-month coverage identified by view" },
                    { icon: <MapPin size={13} />, label: "Search Console and GBP — territory scope and unavailable periods shown explicitly" },
                  ].map((item) => (
                    <div key={item.label} className="flex items-start gap-2">
                      <span className="mt-0.5 flex-shrink-0" style={{ color: "oklch(0.68 0.20 140)" }}>{item.icon}</span>
              ...
              <div className="p-5">
                <div className="text-xs font-semibold tracking-widest uppercase mb-3" style={{ color: "oklch(0.52 0.016 80)", fontFamily: "Inter, sans-serif" }}>Territory context</div>
                <div className="space-y-3 text-xs" style={{ color: "oklch(0.35 0.015 65)", fontFamily: "Inter, sans-serif", lineHeight: 1.5 }}>
                  <p><strong>Reporting type:</strong> Franchise territory.</p>
                  <p><strong>Corporate classification:</strong> Not asserted in this portal. Unmapped network or corporate records stay outside this territory’s active aggregates unless an approved source mapping includes them.</p>
                  <p><strong>Availability rule:</strong> The dashboard labels incomplete or unavailable source periods instead of filling them with zeroes.</p>
                </div>
              </div>
            </div>
            ...
            <div className="px-5 py-3 flex items-center justify-between flex-wrap gap-3" style={{ background: "oklch(0.97 0.012 80)" }}>
              <span className="text-xs" style={{ color: "oklch(0.65 0.010 80)", fontFamily: "Inter, sans-serif" }}>
                Source labels and coverage details appear within each reporting view.
              </span>
              <div className="flex items-center gap-3 flex-wrap">
                {loc.fullReportUrl && (
                  ...
                  >
                    <FileText size={12} /> Historical Report
                  </a>
                )}
                ...
                <Link
                  href={`/dashboard/${loc.id}`}
                  className="text-xs flex items-center gap-1 font-semibold transition-opacity hover:opacity-70"
                  style={{ color: "oklch(0.42 0.13 140)", fontFamily: "Inter, sans-serif", textDecoration: "none" }}
                >
                  View Dashboard →
                </Link>
              </div>
