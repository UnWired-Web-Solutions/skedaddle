          ...
          {isLoading ? (
            ...
            <>
              <div className="rounded-sm border p-4 mb-4 text-sm" style={{ borderColor: MIST, background: "oklch(0.975 0.008 80)", color: "oklch(0.40 0.015 65)" }}>
                <strong style={{ color: FOREST }}>Source:</strong> Approved Google Drive workbook · <strong>{workbook.reportingWindow.label}</strong> · active snapshot <strong>{workbook.activeRun.status}</strong>. Every territory rank uses this same completed-month window. Values are accepted work-order aggregates and recorded pre-tax invoice values, not Salesforce API data, recognized revenue, or conversion data. {workbook.activeRun.status === "partial" ? `${workbook.activeRun.rowsRejected.toLocaleString()} source rows were explicitly excluded from canonical territory aggregates.` : ""}
              </div>
