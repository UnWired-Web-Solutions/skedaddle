...
| Local portal login | Client-side credential map/sessionStorage | Replace with server-validated users, hashed passwords or managed identity, secure HttpOnly cookies, and role/location authorization | Test that every franchise user is restricted to its own territory |
...
| Salesforce | Direct integration code; credentials still pending | Configure only when Barry/UWS supplies an approved Connected App | Do not substitute exports, estimates, or placeholder sales data |
