...
- [ ] Review and implement the user-supplied attached specification, preserving source provenance, public-procedure compatibility, and production verification requirements
- [x] Replace the browser-only local-auth trust model with a server-backed signed local-session architecture on a separate branch; do not merge or deploy pending approved secret and production smoke-test plan — staged signed HTTP-only cookie, server-side account resolution, client session discovery, expiry/tamper/logout/removal regressions, and no production rollout
- [x] Enforce server-side portal, territory, and administrator authorization boundaries while retaining public login, session discovery, logout, and health procedures — staged active-router authorization coverage rejects anonymous and cross-territory access while permitting the correct role
- [x] Add aggregate-only matched-period workbook reporting to Strategy Reports and Proposals, repair ordered migration validation, and produce the requested non-deployed evidence handoff — exact 2025-07 through 2026-06 active workbook aggregate loader, labelled historical fallback, currency isolation, static migration ordering test, and staged rollout handoff complete
- [ ] Provide a dedicated managed `LOCAL_AUTH_SESSION_SECRET`, an isolated disposable MySQL/TiDB database credential for full clean-install execution, and approval for the required production smoke matrix before merge or deployment
...
- [x] Add a server-only administrator credential-rotation override that preserves the existing managed franchise registry, validate it, and remove it only after the primary registry is confirmed live — temporary recovery safeguard was validated and then removed after the primary registry passed independent production sign-in
- [x] Perform the user-approved controlled production primary-registry check; remove the temporary admin override if authentication remains available and restore it immediately if not — primary registry authenticated successfully in production after a fresh session sign-in; no restore was required
- [x] Change the managed administrator password to the user-approved value and validate the updated local and production login without exposing it — primary-registry local regression and fresh production sign-in passed without credential output
- [x] Force a fresh server revision after the user-approved managed-secret update and verify that production reads the current administrator credential before retiring the interim override — explicit environment contract release verified live; active authentication now uses the primary managed registry only
...
- [ ] Google Analytics page performance (sessions, engagement, key events) — direct and durable completed-month engagement delivered; key-event counts remain unavailable pending a network-wide approved definition
...
- [x] Add coverage-aware durable GA4 engagement metrics for completed monthly page snapshots; show key events as unavailable pending a network-wide approved event definition — completed-month direct engagement is persisted separately from YTD; key events remain unavailable pending governance approval
...
- [x] Audit existing source-backed ownership/context fields; display only verified corporate-versus-franchise classifications and otherwise use an explicit unknown state — mapping supports franchise territory context only; unmapped corporate/network records remain outside active territorial aggregates
- [x] Audit and retire active consumers of static territory revenue, jobs, species, session trend, and network-rank values so source-backed or unavailable states replace them — active Home, Location Detail, action-plan, Network, Dashboard, Analytics, and report paths now use source-aware labels or explicitly retained historical snapshots
- [x] Replace Home page static territory KPI cards and dashboard-completeness claims with non-performance territory context and source-availability guidance — local authenticated overview verified
- [x] Replace Location Detail’s static species, trend, update-date, and dashboard-completeness claims with source-aware reporting guidance and the verified franchise-territory context — local Milwaukee detail verified
...
- [x] Verify the checkpointed Home, Location Detail, and 90-day action-plan source-aware presentation on production; initial checkpoint `c7ef987c` served the prior static client artifact — retry checkpoint `327e9f97` propagated; published Home and Milwaukee detail/action plan verified without static KPI, T12 demand, closed-job, or ownership claims
...
- [x] Publish and verify internal GPT-5.5 strategy-report generation in production, including bounded completion and the exact-draft output flow — post-capacity-retry Ottawa run completed in approximately 149 seconds; exact 21-page production PDF was fully reviewed with source and conversion safeguards intact
...
- [x] Update portal and report source disclosures to identify the Drive workbook rather than a live Salesforce API — Analytics, territory dashboard, and exact Ottawa report now identify the Drive workbook; report historical sales values are explicitly labelled as legacy snapshots
- [x] Remove legacy Salesforce snapshot calculations from territory dashboards and reports so no hard-coded inspection, closed-job, revenue, or close-rate value is presented as fresh workbook data — the legacy analytics close-rate procedure and dashboard/report conversion panels are retired; historical revenue/species/city material remains only with an explicit legacy-snapshot disclosure
- [x] Add a read-only completed-month workbook performance query for territory work-order volume, invoice-value context, species, and cities without defining inspections, closed jobs, revenue, or close rate — active partial snapshot response verified for Hamilton; all values remain source-labelled and currency-safe
- [x] Add read-only portal status and territory-period procedures for the active workbook run without exposing raw addresses, IDs, or salesperson fields — active-run status and territory-performance contracts, dashboard, and Analytics provenance were verified without returning source rows or sensitive operational fields
- [x] Verify database reconciliation, endpoints, authenticated UI, reports, tests, and production build before activation — final release passed TypeScript, 164 tests with 11 intentional skips, production build, endpoint checks, authenticated Dashboard/Analytics review, and a fresh 14-page Ottawa exact-PDF inspection
- [x] Checkpoint the verified workbook workflow, push canonical GitHub `main`, and update session memory — checkpoint `e08d9b68` deployed successfully; canonical `UnWired-Web-Solutions/skedaddle:main` was normally pushed and verified at `e08d9b68bcdd35ab9d07fc4327e43a20345bd0d2`
- [x] Observe and document the first naturally changed Drive revision completing the bounded low-memory production import; no source workbook mutation was used — scheduled run `930004` activated a partial snapshot in 26.745 seconds after a natural Drive revision
- [x] Make the full regression command deterministic by preventing concurrent suites from exhausting the shared strategy-report database timing budget — isolated report tests and single-worker suite confirmed contention; standard `pnpm test` now serializes Vitest workers and passed 179 tests with 11 intentional skips
