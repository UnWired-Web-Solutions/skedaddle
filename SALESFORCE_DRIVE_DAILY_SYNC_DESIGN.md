...
The Network page uses the parallel aggregate-only `salesforceWorkbook.getNetworkPerformance` contract. It ranks accepted territory aggregates only within the same currency code and labels its amount as **Recorded Invoice Value (Pre-Tax)**. It does not represent the active workbook as a trailing-12-month revenue feed, does not fall back to retained static snapshots, and exposes the active partial-source status and explicit exclusions. Territories without an accepted active-workbook aggregate are not zero-filled into the ranking.

Production review confirmed the authenticated Network page renders this active-workbook summary. It visibly shows separate USD and CAD rank sequences, the active `partial` state and 48,524 explicit exclusions, and only the aggregate-safe work-order, invoice-row, and recorded pre-tax invoice-value fields. The former static Salesforce T12-revenue table, job average, and top-species claims no longer appear in the active ranking panel.
