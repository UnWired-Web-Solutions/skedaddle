...
export const analyticsRouter = router({
  ...
  getGA4ReadyTerritories: adminProcedure.query(() => getGA4ReadyTerritories()),
   ...
   */
  getGBPIntegrationStatus: adminProcedure.query(() => ({
   ...
   */
  getSearchConsoleReadyTerritories: adminProcedure.query(() => {
   ...
   */
  getTerritories: portalProcedure.query(async () => {
   ...
   */
  getDateRange: adminProcedure.query(async () => {
    const db = await getDb();
    if (!db) return {
   ...
   */
  getInsights: territoryProcedure
    .input(z.object({
      year: z.number(),
      month: z.number().min(1).max(12),
